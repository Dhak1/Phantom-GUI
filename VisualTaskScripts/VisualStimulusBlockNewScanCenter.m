function VisualStimulusBlockNewSB(Grayperiod,Nblocks,OnDur,OffDur,Endperiod,viewfield,TR,dotsize) 
...
%           OffDur: the duration of off period within each block  
%           Endperiod: additional gray period post stimulus  
%           viewfield: 'left', left half viewfield; 
%                      'right', right half viewfield;  
%                      'full', full viewfield  
%           TR: TR  
%           dotsize: the size of red dots  

% example VisualStimulusBlockNew2(20,6,8,18,6,'full',0.754,20);
          
%% set up saving path  
fpath='/Users/danihak/Documents/fmri_res/'; 
...
Tdt = 1;  
Task = [zeros(Grayperiod,1); repmat([ones(OnDur,1); zeros(OffDur,1)], Nblocks, 1); zeros(Endperiod,1)];  
...
RA = num2str(sum(~isnan(rt))/length(rt)*100);  
mRT = num2str(nanmean(rt)*1000);  
save(fname,'Task','LumContrast','TR','RA','mRT','FlipFreq','keyval','switches','TRtimes');
