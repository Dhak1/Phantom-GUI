...
[handles.data, cwd, name,fn] = loadPhanIm;
...
% handles    structure with handles and user data (see GUIDATA)
set(findobj('Tag','statusText'),'String','Loading Data...')% Update handles structure
[handles.volume, name,fn] = loadPhanIm;
...
imshow(handles.image,[0 max(max(squeeze(handles.volume(:,:, round(handles.slices/2), 1))))]);
%a = dir('*.csv');
...
set(findobj('Tag','filterToggle'),'Value', 0);
set(findobj('Tag','detrendTog'),'Value', 0);
handles.volume = handles.volume(:,:,handles.sliceNum,:);
guidata(hObject, handles);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[handles.T2Vol,cwd, name,fn]=loadPhanIm;
...
%     end
    ...
    handles.quadrantIDMatrix = quadrantID2(handles.T2Map,quadrantNum);
    sizeQuadrantIDMatrix = size(handles.quadrantIDMatrix);
    
    %standard method for displaying the quadrant boundaries as overlays-can be
    %shortened
...
%         SliceNum = i;
        ...
        image(handles.T2Map(:,:,SliceNum,2));colormap(gray)
        hold on
        ...
        for armRotation = 0:360/quadrantNum:359
            for x = 1:T2VolSize(1,1)
                x1 = ceil(x*cosd(handles.quadrantIDMatrix(SliceNum,1)+armRotation)+handles.quadrantIDMatrix(SliceNum,2));
                y1 = ceil(x*sind(handles.quadrantIDMatrix(SliceNum,1)+armRotation)+handles.quadrantIDMatrix(SliceNum,3));
                x1 = int16(x1);
                y1 = int16(y1);
